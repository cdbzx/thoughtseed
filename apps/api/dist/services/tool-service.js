...
exports.buildToolPlan = buildToolPlan;
const tool_learning_service_1 = require("./tool-learning-service");
const task_type_service_1 = require("./task-type-service");
const tool_run_service_1 = require("./tool-run-service");
const TOOLS = [
    {
        id: 'capture-context',
        name: '上下文提取器',
...
async function buildToolPlan(input) {
    ...
    const taskType = (0, task_type_service_1.detectTaskType)(input);
    const [history, learnedRules, preferences, typePreferences] = await Promise.all([
        input.ideaId ? (0, tool_run_service_1.listIdeaToolRuns)(input.ideaId) : Promise.resolve([]),
        (0, tool_run_service_1.listRecentLearnedRules)(),
        (0, tool_learning_service_1.listToolPreferences)('global'),
        (0, tool_learning_service_1.listToolPreferences)(taskType),
    ]);
    ...
    if (topTypePreference && topTypePreference.preference > 0) {
        recommendedTools.push({
            ...
            reason: `在${(0, task_type_service_1.taskTypeLabel)(taskType)}任务里，这个工具最近更常被采纳（偏好分 ${topTypePreference.preference}，信号 ${topTypePreference.signalCount} 次）。`,
    ...
    for (const item of recommendedTools) {
        ...
        if (!existing || item.score > existing.score)
            deduped.set(item.toolId, item);
    }
    const finalTools = Array.from(deduped.values()).sort((a, b) => b.score - a.score);
    const explanation = [
        `当前任务被识别为：${(0, task_type_service_1.taskTypeLabel)(taskType)}。`,
