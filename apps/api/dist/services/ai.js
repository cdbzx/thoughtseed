...
exports.expandIdeaWithAI = expandIdeaWithAI;
function fallbackExpandIdea(input) {
...
async function expandIdeaWithAI(input) {
    ...
    const prompt = `你是 ThoughtSeed 的灵感整理助手。请把用户输入整理为结构化结果，并严格输出 JSON，不要包含 markdown 代码块，不要输出额外解释。\n\n字段要求：\n- title: 适合展示的短标题\n- summary: 1-2 句摘要\n- problem: 用户/场景的核心问题\n- audience: 目标用户\n- value: 这条想法的潜在价值\n- nextSteps: 下一步建议\n\n用户输入：${input.content}\n标签：${input.tags?.join(', ') || '无'}`;
        ...
        const response = await fetch(endpoint, {
            ...
            body: JSON.stringify({
                ...
                messages: [
                    {
                        role: 'system',
                        content: '你擅长将中文灵感整理成简洁、可执行、结构清晰的 JSON。必须只输出合法 JSON 对象。',
                    },
                    {
                        role: 'user',
                        content: prompt,
